...
export class ContactComponent {
  ...
  contactusForm = new FormGroup({
    ...
    description: new FormControl("",Validators.compose([Validators.required,Validators.minLength(20)])),
  });
  submitForm(): void {
    this.contactPostSubscription = this.apis.contactPostApi(this.contactusForm.value).subscribe(()=>{
      this.alert.displayNotification({type:'success', msg:'Submitted Your Response....'})
    })
